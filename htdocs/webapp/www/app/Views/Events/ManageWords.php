...
<div class="manage_container">
    ...
    <div class="manage_body">
        <div class="manage_chapters">
            ...
            <ul>
                <?php foreach ($event->words as $word): ?>
                    <?php
                    $chapter = $word->wordID;
                    $chapData = $chapters[$chapter];

                    if(!empty($chapData)) {
                        $member = $members->find($chapData["memberID"]);
                        $name = $chapData["memberName"];
                    }
                    ?>
        ...
        <div class="manage_members">
            ...
            <ul>
                <?php foreach ($members as $member):?>
                    <?php
                    $words = $event->words;
                    $assignedChapters = $member->chapters
                        ->filter(function($chap) use($event) {
                            return $chap->eventID == $event->eventID;
                        })
                        ->map(function($chap) use($words) {
                            return $chap->chapter;
                        });
                    $chapterNumbers = $assignedChapters->toArray();
                    sort($chapterNumbers);
                    ?>
                    <li>
                        <div class="member_username" data-userid="<?php echo $member->memberID ?>">
                            ...
                            (<span><?php echo $assignedChapters->count(); ?></span>)
                        ...
                        <div class="member_chapters" <?php echo !$assignedChapters->isEmpty() ? "style='display:block'" : "" ?>>
                            &nbsp;
