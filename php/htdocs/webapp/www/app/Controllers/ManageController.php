...
class ManageController extends Controller {
    ...
    public function manage($eventID)
    {
        ...
        if ($event) {
            ...
            foreach ($chapters as $chapter) {
                ...
                $tmp["verbCheck"] = $chapter["verbCheck"] ? (array)json_decode($chapter["verbCheck"], true) : [];
                $tmp["kwCheck"] = $chapter["kwCheck"] ? (array)json_decode($chapter["kwCheck"], true) : [];
                $tmp["crCheck"] = $chapter["crCheck"] ? (array)json_decode($chapter["crCheck"], true) : [];
                $tmp["peerCheck"] = $chapter["peerCheck"] ? (array)json_decode($chapter["peerCheck"], true) : [];
                $tmp["otherCheck"] = $chapter["otherCheck"] ? (array)json_decode($chapter["otherCheck"], true) : [];
    ...
    public function manageTw($eventID)
    {
        ...
        if ($event) {
            ...
            foreach ($data["word_groups"] as $group) {
                $words = $group->words ? (array)json_decode($group->words, true) : [];
    ...
    public function manageWords($eventID)
    {
        ...
        if ($event) {
            ...
            foreach ($chapters as $chapter) {
                ...
                $otherCheck = $chapter["otherCheck"] ? (array)json_decode($chapter["otherCheck"], true) : [];
                $peerCheck = $chapter["peerCheck"] ? (array)json_decode($chapter["peerCheck"], true) : [];
        ...
        if ($eventID !== null && $memberID !== null) {
            ...
            if ($event) {
                ...
                if ($manageMode == "l2") {
                    ...
                    if (Tools::isHelp($mode)) {
                        $otherCheck = $member->otherCheck ? (array)json_decode($member->otherCheck, true) : [];
                    ...
                    if ($confirm < MoveBackConfirm::MOVE_STEP) {
                        if (Tools::isHelp($mode) && $currentStep != EventSteps::PRAY) {
                            ...
                            if ($prevStep == EventSteps::FINAL_REVIEW || $prevStep == EventSteps::CONTENT_REVIEW) {
                                $crCheck = $member->crCheck ? (array)json_decode($member->crCheck, true) : [];
                                ...
                                $kwCheck = $member->kwCheck ? (array)json_decode($member->kwCheck, true) : [];
                            ...
                            if ($prevStep == EventCheckSteps::PEER_REVIEW) {
                                $peerCheck = $member->peerCheck ? (array)json_decode($member->peerCheck, true) : [];
            ...
            case EventSteps::CONSUME:
                ...
                if ($mode != "tn") {
                    if (in_array($mode, ["obs","odbsun"])) {
                        ...
                        $verbCheck = $member->verbCheck ? (array)json_decode($member->verbCheck, true) : [];
    ...
    private function getMemberCurrentStep($member, $chapter, $chapterDone, $mode, $manageMode) {
        if ($manageMode == "l3") {
            $peer = $member->peerCheck ? (array)json_decode($member->peerCheck, true) : [];
            ...
            $peer = $member->peerCheck ? (array)json_decode($member->peerCheck, true) : [];
            $kw = $member->kwCheck ? (array)json_decode($member->kwCheck, true) : [];
            $cr = $member->crCheck ? (array)json_decode($member->crCheck, true) : [];
            ...
            $kw = $member->kwCheck ? (array)json_decode($member->kwCheck, true) : [];
            $cr = $member->crCheck ? (array)json_decode($member->crCheck, true) : [];
            $other = $member->otherCheck ? (array)json_decode($member->otherCheck, true) : [];
    ...
    private function checkHasTranslations($translations, $manageMode) {
        if ($manageMode == "l1") {
            ...
            $checkerTranslations = $translations->filter(function($item) use ($checker) {
                $verses = $item->translatedVerses ? (array)json_decode($item->translatedVerses, true) : [];
            ...
            if ($event) {
                if ($this->isAdmin($event)) {
                    ...
                    foreach ($chapters as $chap) {
                        ...
                        $tmp["chunks"] = $chap["chunks"] ? (array)json_decode($chap["chunks"], true) : [];
                        $tmp["done"] = $chap["done"];
                            ...
                            if (!$hasTranslations || $confirm == AssignChapterConfirm::ASSIGN) {
                                if ($manageMode == "l1") {
                                    if ($data["chapters"][$chapter]["memberID"] == $memberID) {
                                        ...
                                        $verbCheck = $translator->pivot->verbCheck ? (array)json_decode($translator->pivot->verbCheck, true) : [];
                                        $peerCheck = $translator->pivot->peerCheck ? (array)json_decode($translator->pivot->peerCheck, true) : [];
                                        $kwCheck = $translator->pivot->kwCheck ? (array)json_decode($translator->pivot->kwCheck, true) : [];
                                        $crCheck = $translator->pivot->crCheck ? (array)json_decode($translator->pivot->crCheck, true) : [];
                                        $otherCheck = $translator->pivot->otherCheck ? (array)json_decode($translator->pivot->otherCheck, true) : [];
                                        if (array_key_exists($chapter, $verbCheck)) {
                                            unset($verbCheck[$chapter]);
                                    ...
                                    if ($data["chapters"][$chapter]["l2memberID"] == $memberID) {
                                        ...
                                        $peerCheck = $checker->pivot->peerCheck ? (array)json_decode($checker->pivot->peerCheck, true) : [];
                                        $kwCheck = $checker->pivot->kwCheck ? (array)json_decode($checker->pivot->kwCheck, true) : [];
                                        $crCheck = $checker->pivot->crCheck ? (array)json_decode($checker->pivot->crCheck, true) : [];
                                        if (array_key_exists($chapter, $peerCheck)) {
                                            unset($peerCheck[$chapter]);
                                            $trPostData["peerCheck"] = json_encode($peerCheck);
                                        }
    ...
    public function addEventMember()
    {
        ...
        /*if ($userType == EventMembers::L2_CHECKER || $userType == EventMembers::L3_CHECKER) {
            $education = $appliedMember->profile->education ? (array)json_decode($appliedMember->profile->education) : [];
            if (empty($education)) {
            ...
            $ed_area = $appliedMember->profile->ed_area ? (array)json_decode($appliedMember->profile->ed_area) : [];
            if (empty($ed_area))
            ...
            $church_role = $appliedMember->profile->church_role ? (array)json_decode($appliedMember->profile->church_role) : [];
        ...
        if ($eventID && !empty($group)) {
            ...
            if ($event) {
                ...
                foreach ($groups as $gr) {
                    $elm = $gr->words ? (array)json_decode($gr->words, true) : [];
