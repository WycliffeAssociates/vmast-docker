...
class ScriptureProgress
{
    public static function calculateEventProgress($event, $progressOnly = false) {
        ...
        foreach ($data["chapters"] as $key => $chapter) {
            ...
            $verbCheck = $memberSteps[$chapter["memberID"]]["verbCheck"] ? json_decode($memberSteps[$chapter["memberID"]]["verbCheck"], true) : [];
            $peerCheck = $memberSteps[$chapter["memberID"]]["peerCheck"] ? json_decode($memberSteps[$chapter["memberID"]]["peerCheck"], true) : [];
            $kwCheck = $memberSteps[$chapter["memberID"]]["kwCheck"] ? json_decode($memberSteps[$chapter["memberID"]]["kwCheck"], true) : [];
            $crCheck = $memberSteps[$chapter["memberID"]]["crCheck"] ? json_decode($memberSteps[$chapter["memberID"]]["crCheck"], true) : [];
            ...
            } elseif ($event->inputMode == InputMode::SPEECH_TO_TEXT) {
